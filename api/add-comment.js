// POST /api/add-comment  { name, email, section, text, parentId }
// Proxies to Apps Script's addComment(name, email, section, text, parentId).
...
module.exports = async function handler(req, res) {
  ...
  try {
    const { name, email, section, text, parentId } = req.body || {};
    const data = await callAppsScript('addComment', [name, email, section, text, parentId]);
